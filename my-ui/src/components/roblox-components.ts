...
const registry: Record<string, ComponentResolver> = {
  ScreenGui: (component, resolve) => ({ ClassName: 'ScreenGui', Name: String(component.Props?.Name ?? 'ComponentPreviewGui'), Children: (component.Children ?? []).map(resolve) }),
  MoneyDisplay: (component, resolve) => {
    const props = component.Props ?? {}
    return {
      ClassName: 'Frame', Name: String(props.Name ?? 'MoneyDisplay'), Size: props.Size ?? { X: { Scale: 0, Offset: 180 }, Y: { Scale: 0, Offset: 48 } },
      BackgroundTransparency: 1,
      Children: [{ ClassName: 'TextLabel', Name: 'MoneyText', Text: props.Text ?? ({ __kind: 'RobloxBinding', Path: 'Player.Money', Format: (value: unknown) => `$${typeof value === 'number' ? value : 0}` } satisfies RobloxBinding), TextSize: props.TextSize ?? 20, TextColor3: props.TextColor3 ?? { R: 0.96, G: 0.65, B: 0.14 }, Size: { X: { Scale: 1, Offset: 0 }, Y: { Scale: 1, Offset: 0 } }, Children: (component.Slots?.content ?? component.Children ?? []).map(resolve) }],
    }
  },
  InventoryCard: (component, resolve) => {
    const props = component.Props ?? {}
    return {
      ClassName: 'Frame', Name: String(props.Name ?? 'InventoryCard'), Size: props.Size ?? { X: { Scale: 0, Offset: 240 }, Y: { Scale: 0, Offset: 120 } }, BackgroundColor3: props.BackgroundColor3 ?? { R: 0.067, G: 0.078, B: 0.102 },
      Children: [{ ClassName: 'TextLabel', Name: 'InventoryText', Text: props.Text ?? 'Inventory', TextColor3: props.TextColor3 ?? { R: 0.96, G: 0.97, B: 0.98 }, Size: { X: { Scale: 1, Offset: 0 }, Y: { Scale: 0, Offset: 32 } } }, ...(component.Slots?.body ?? component.Children ?? []).map(resolve), { ClassName: 'UICorner', CornerRadius: { Scale: 0, Offset: 10 } }],
    }
  },
  PurchaseButton: (component) => {
    const props = component.Props ?? {}
    const variant = props.variant === 'danger' ? { R: 0.85, G: 0.18, B: 0.18 } : { R: 0.96, G: 0.65, B: 0.14 }
    return { ClassName: 'TextButton', Name: String(props.Name ?? 'PurchaseButton'), Text: props.Text ?? 'Buy Bat', TextSize: props.TextSize ?? 16, TextColor3: props.TextColor3 ?? { R: 0.07, G: 0.09, B: 0.13 }, BackgroundColor3: props.BackgroundColor3 ?? variant, Size: props.Size ?? { X: { Scale: 0, Offset: 180 }, Y: { Scale: 0, Offset: 42 } }, Children: [{ ClassName: 'UICorner', CornerRadius: { Scale: 0, Offset: 8 } }] }
  },
  EMSAlert: (component, resolve) => {
    const props = component.Props ?? {}
    return { ClassName: 'Frame', Name: String(props.Name ?? 'EMSAlert'), Visible: props.Visible ?? ({ __kind: 'RobloxBinding', Path: 'EMS.HasActiveCall' } satisfies RobloxBinding), BackgroundColor3: props.BackgroundColor3 ?? { R: 0.85, G: 0.18, B: 0.18 }, Size: props.Size ?? { X: { Scale: 0, Offset: 260 }, Y: { Scale: 0, Offset: 52 } }, Children: [{ ClassName: 'TextLabel', Name: 'EMSAlertText', Text: props.Text ?? 'EMS call received', TextColor3: { R: 0.96, G: 0.97, B: 0.98 }, Size: { X: { Scale: 1, Offset: 0 }, Y: { Scale: 1, Offset: 0 } }, Children: (component.Slots?.content ?? component.Children ?? []).map(resolve) }, { ClassName: 'UICorner', CornerRadius: { Scale: 0, Offset: 8 } }] }
  },
}
...
export const previewComponentDocument = resolveRobloxComponents({ Component: 'ScreenGui', Props: { Name: 'ComponentPreviewGui' }, Children: [
